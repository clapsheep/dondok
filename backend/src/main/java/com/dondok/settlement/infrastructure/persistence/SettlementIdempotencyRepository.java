...
@Repository
public class SettlementIdempotencyRepository {
    private final JdbcTemplate jdbcTemplate;

    public SettlementIdempotencyRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public Claim claim(UUID userId, UUID bookId, String scope, String key, String requestHash, Instant now) {
    ...
    public void complete(UUID userId, String scope, String key, UUID resourceId, Instant now) {
        jdbcTemplate.update("""
                update api_idempotency
                   set processing_status = 'COMPLETED', response_status = 201,
                       resource_id = ?, completed_at = ?
                 where actor_user_id = ? and endpoint_scope = ? and idempotency_key = ?
                """, resourceId, Timestamp.from(now), userId, scope, key);
    }
