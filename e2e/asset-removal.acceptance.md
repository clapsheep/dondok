...
이 문서는 `docs/project-context.md`의 D-007·D-034·D-036·D-051, `docs/api/openapi.yaml` 0.8, `docs/architecture/backend-design.md` §8, `docs/architecture/database-design.md`, `docs/design/ux-guidelines.md`의 자산 삭제·보관 계약을 Playwright 인수 조건으로 연결한다.
...
- 현재 잔액과 미결제 카드 명세는 warning이며 삭제·보관을 막지 않는다.
...
2. 무이력 계좌 편집에서 `처리 방법 확인`을 눌러 `완전 삭제`, 이력 0건, 최종 삭제 행동을 확인하고 UI로 실행한다.
...
7. `보관 자산 N개`를 열어 compact 행과 과거 거래 원장을 확인한다. 편집 기어·복원·삭제·보관 행동은 없어야 한다.
...
4. A dialog는 닫히지 않고 `처리 방법이 달라졌어요` 경고에 focus한다. 최종 행동은 사라지고 `최신 내용 다시 확인`만 남는다.
