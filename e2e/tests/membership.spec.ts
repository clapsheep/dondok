...
test('가계부 생성자가 초대한 구성원과 서로의 계좌를 함께 관리하고 이체한다', async ({ page, request }) => {
  ...
  await expect(sourcePicker.picker.getByRole('button', { name: new RegExp(`^${accounts.source.name}, 계좌, 나,`) })).toBeVisible()
  await expect(sourcePicker.picker.getByRole('button', { name: new RegExp(`^${accounts.destination.name}, 계좌, ${memberName},`) })).toHaveCount(0)
  ...
  await expect(sourcePicker.picker.getByRole('button', { name: new RegExp(`^${accounts.destination.name}, 계좌, ${memberName},`) })).toBeVisible()
  ...
  await expect(reopenedSourcePicker.picker.getByRole('button', { name: new RegExp(`^${accounts.destination.name}, 계좌, ${memberName},`) })).toHaveCount(0)
  await page.keyboard.press('Escape')
