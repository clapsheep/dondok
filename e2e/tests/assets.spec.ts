...
test('새 가계부의 기본 자산과 이름을 포함한 빠른 자산 등록이 목록에서 즉시 상세로 이어진다', async ({ page, request }) => {
  ...
  await page.getByLabel('대출 기관', { exact: true }).click()
  await page.getByRole('button', { name: 'KB캐피탈', exact: true }).click()
  ...
  await expect(row).toContainText('KB캐피탈')
...
test('자산 이름을 직접 저장하거나 비워 두면 고정 종류 이름에 순번을 붙인다', async ({ page, request }) => {
  ...
  await expect(page.getByLabel('증권사', { exact: true })).toContainText('기타 증권사')
  await page.getByLabel('증권사', { exact: true }).click()
  await page.getByRole('button', { name: '키움증권', exact: true }).click()
  await page.getByRole('button', { name: '변경 저장' }).click()
  await expect(page.getByRole('status')).toContainText('자산 정보를 변경했어요')
  await expect(page.locator('p:visible').filter({ hasText: '키움증권 · 투자' }).first()).toBeVisible()
...
test('신용카드 빠른 등록은 필수 정산 정보만 받고 자동 정산은 상세에서 설정한다', async ({ page, request }) => {
  ...
  await expectQuickCreateFields(page, '기준일 잔액')
  await expect(page.getByRole('switch', { name: /^결제일에 자동 정산/ })).toHaveCount(0)
  await page.getByLabel('카드사', { exact: true }).click()
  await page.locator('[data-slot="popover-content"]').getByRole('button', { name: '신한카드', exact: true }).click()
  ...
  await expect(row).toBeVisible()
  await expect(row).toContainText('신한카드')
  await expectCardPaymentAmounts(row, { nearest: '180,000원' })
  await row.getByRole('link').click()
