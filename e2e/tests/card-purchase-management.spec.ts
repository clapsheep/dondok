...
test('카드 구매 기록 정정은 저장 확인 dialog 뒤 변경된 구매와 명세를 원 구매 대신 반영한다', async ({ page, request }, testInfo) => {
  const month = currentMonthInSeoul()
  const purchaseDate = todayInSeoul()
  ...
  let confirmation = page.getByRole('dialog', { name: '정정 내용을 저장할까요?' })
  await expect(confirmation).toBeVisible()
  await expect(confirmation.getByRole('heading', { name: '정정 내용을 저장할까요?' })).toBeFocused()
  await expect(confirmation).toContainText('90,000원')
  await expect(confirmation).toContainText('→ 60,000원')
  await expect(confirmation.getByText('미결제 카드 금액 감소').locator('..')).toContainText('30,000원')
  ...
  await page.getByRole('button', { name: '정정 저장' }).click()
  confirmation = page.getByRole('dialog', { name: '정정 내용을 저장할까요?' })
  await expect(confirmation.getByText('미결제 카드 금액 감소').locator('..')).toContainText('25,000원')
  await expectTouchTarget(confirmation.getByRole('button', { name: '저장', exact: true }), '정정 저장 확인')
