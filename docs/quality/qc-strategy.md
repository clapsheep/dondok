...
Playwright는 `회원가입 → 이메일 인증 → 서버 세션 로그인 → 로그아웃·재로그인 → 비밀번호 재설정 메일 → 새 비밀번호 저장·로그인`과 `가계부 생성 → 초대 URL·코드 발급 → 다른 사용자 미리보기·수락 → 양쪽 구성원 확인` 흐름을 모바일 Chrome, iPad 세로, 데스크톱 Chrome에서 검증한다. PostgreSQL integration test는 동일 초대의 동시 수락, 동일 사용자의 서로 다른 가계부 동시 수락, 동일 사용자의 가계부 동시 생성을 검증한다.
...
- 소유자별 보기 URL을 직접 열거나 새로고침·viewport를 변경해도 선택이 유지되고 유효하지 않은 owner 값은 전체로 복구되며, 0건은 전체 가계부 onboarding과 다른 compact 빈 상태로 표시
...
- 자산 행은 종류·소유자만을 위한 두 번째 줄이나 별도 metadata 열을 만들지 않고 identity를 한 줄로 유지하며, 필요한 metadata는 자산명 바로 뒤에 두고 기본 이름의 중복 종류를 생략하며 custom 이름에만 종류를 표시함
- 전체 보기의 같은 identity 줄에는 `나`·`공동`·다른 구성원명이 보이지만 소유자별 보기에서는 선택된 소유자를 반복하지 않고, 시각적으로 생략해도 링크의 접근 가능한 이름에는 전체 종류·소유자가 유지됨
- 현금·기타·계좌·적금·마이너스통장이 하나의 `자금` 그룹에 한 번씩 표시되고, 그룹 signed 현재 합계와 각 signed 현재 금액이 가장 오른쪽 열에 정렬되며 음수 마이너스통장을 `부채` 절댓값으로 바꾸지 않음
...
- 자금·카드 등 서로 다른 자산 그룹 사이는 모바일 28px, iPad 32px, 데스크톱 36px의 일관된 빈 여백과 전체 폭 1px 시작선으로 구분되고, 시작선은 내부 구분선보다 강하지만 배경·좌우 테두리·둥근 모서리·그림자를 만들지 않음
...
- 일반 이체 선택기에는 활성 `BANK` 계좌만 표시되고 현금·신용카드·체크카드·적금·마이너스 통장은 표시되지 않으며, 계좌가 두 개 미만이면 저장할 수 없음
