...
- 마지막 지출 자산 기본값은 가계부·사용자 범위, 활성 자산 검증, 수입·이체 비덮어쓰기와 명시적 진입 문맥 우선을 회귀 테스트한다.
...
- 자산 현황의 전체·공동 소유·각 구성원 보기가 같은 소유 marker 부분집합으로 순자산·총자산·총부채·가장 가까운 두 카드 결제 금액, 그룹 합계와 자산 행을 함께 바꾸고 활성 `n/50`은 전체 기준을 유지
...
- 순자산이 전체 자산 요약의 핵심으로 한 번 표시되고 총자산·총부채와 가까운 결제·그다음 결제 합계가 의미별로 구분되며, 그룹이 모바일·iPad·데스크톱에서 한 열의 고정 순서를 유지
...
- 카드 그룹은 `가까운 결제 합계 | 그다음 결제 합계`, 신용카드 행은 실제 `M월 d일 결제 | M월 d일 결제`를 사용하고, 명세 없는 카드·체크카드는 `결제 예정 없음`을 표시하며 `부채` 문구를 사용하지 않음
...
- 여러 종류의 자산 후보는 compact 종류 필터로 좁혀지고 option마다 종류 icon·이름·자산 종류·소유자 avatar/marker·현재 잔액 또는 가장 가까운 카드 결제 예정액이 보이며, 계좌·카드 전용 후보에는 다른 종류가 섞이지 않음
...
- 분류 순서 이동 핸들은 44px 이상이며 포인터·터치 drag와 키보드 조작을 지원하고, 저장한 순서가 새로고침과 거래 입력 picker에서도 유지됨
- 순서 변경 요청은 현재 방향의 전체 ID·version을 원자적으로 검증해 다른 구성원의 추가·수정·삭제·재정렬이 선행되면 `412`로 전부 거부하고 최신 목록을 다시 보여줌
...
- 일별 보기와 날짜 상세의 설명 있는 수입·지출 행은 보조정보 첫 항목에 분류명을 보여주고, 설명이 없는 행은 분류명을 주 제목으로 보여주며 이체·카드 정산에는 분류를 만들지 않음
...
- 카드 구매 기록 정정은 `정정 저장` 한 번으로 서버 preview를 요청해 확인 dialog를 열고, dialog `취소`는 draft를 보존하며 `저장`만 apply한다. 인라인 중간 적용 단계는 노출하지 않음
...
- 완료된 선결제·자동 정산의 출금 계좌 정정은 금액·결제일·명세 합계를 유지하면서 payment·음수 posting·완료 schedule을 함께 바꾸고 기존 계좌 복원과 새 계좌 차감을 원자적으로 반영
- 출금 계좌 정정은 활성 결제 가능 자산과 최신 statement version만 허용하며 환불 반환이 연결된 결제는 반환 posting 일부만 남기지 않도록 `409`로 거부
...
- 신용카드 picker는 가장 이른 미결제 결제일의 금액을 `결제 예정`으로 표시함
...
- 자산 원장은 모바일과 iPad·데스크톱에서 자산 현황으로 돌아가는 동선을 제공하고, 기준일 잔액 행과 각 거래의 거래 후 잔액을 최신순 cursor·월 구분 안에서 한 번씩 표시함
- 자산 변경 저장에 성공하면 편집 form을 벗어나 해당 자산 원장으로 복귀하고, 완료 메시지와 변경된 자산명·현재 잔액을 즉시 확인할 수 있음
- 활성 자산 원장의 `기록 추가`는 URL을 바꾸지 않는 반응형 dialog를 열고 해당 자산을 기본 선택하며, 초안 닫기 확인에서 계속 작성을 고르면 입력을 보존하고 저장 뒤 dialog만 닫은 채 현재 잔액과 최신 거래 행을 갱신함
- 기준일 당일 이후 실제 posting은 현재 잔액에서 최신순으로 역산되고, 기준일 이전 거래는 기준일 잔액에서 과거 방향으로 이어지며, 체크카드처럼 다른 연결 계좌만 움직인 거래는 선택 자산 잔액을 바꾸지 않음
- 월간 달력·MVP 통계는 요청한 날짜 범위만 집계하고 전체 거래 목록을 먼저 가져오지 않음
- MVP 통계는 선택 월 수입·지출·순액, 카테고리 비중, 같은 연도 1~12월 수입·지출 합계와 현재 구성원 기본값, 각 구성원·전체 전환을 검증
