...
Playwright는 `회원가입 → 이메일 인증 → 서버 세션 로그인 → 로그아웃·재로그인 → 비밀번호 재설정 메일 → 새 비밀번호 저장·로그인`과 `첫 진입 선택 → 가계부 생성 → 6자리 코드·별도 초대 URL 발급 → 다른 사용자 미리보기·수락 → 양쪽 구성원 확인` 흐름을 모바일 Chrome, iPad 세로, 데스크톱 Chrome에서 검증한다. PostgreSQL integration test는 직접 코드 digest 중복 거부, 기존 긴 token 마이그레이션 보존, 동일 초대의 동시 수락, 동일 사용자의 서로 다른 가계부 동시 수락, 동일 사용자의 가계부 동시 생성을 검증한다.
...
- 일반 자산 등록과 계좌 dialog의 최초 금액이 빈 실제 값과 `0` placeholder로 시작하고, 빈 제출과 명시적인 `0` 입력이 모두 `openingBalanceWon: 0`으로 저장
...
- 자산 행은 종류·소유자만을 위한 별도 metadata 열을 만들지 않고 필요한 metadata를 자산명 바로 뒤에 두며, 320~390px에서는 자산명·metadata를 말줄임 없이 감싸고 iPad 이상에서는 한 줄 위계를 유지함
- 전체 보기의 같은 identity 줄에는 `나`·`공동`·다른 구성원명이 보이지만 소유자별 보기에서는 선택된 소유자를 반복하지 않고, 시각적으로 생략해도 링크의 접근 가능한 이름에는 전체 종류·소유자가 유지됨
...
- 자금·카드 등 서로 다른 자산 그룹 사이는 모바일 20px, iPad 32px, 데스크톱 36px의 일관된 빈 여백과 전체 폭 1px 시작선으로 구분되고, 시작선은 내부 구분선보다 강하지만 배경·좌우 테두리·둥근 모서리·그림자를 만들지 않음
- 카드 두 열은 320~390px에서 자산 정보 아래, iPad 이상에서 자산 정보와 같은 행에 정렬되며 DOM 순서·접근 가능한 이름·페이지 overflow 없이 같은 월 의미를 유지
- 320~390px에서 소유자 subnav가 한 줄 내부 스크롤로 유지되고 이름을 말줄임하지 않으며, 첫 자산 그룹이 첫 viewport 안에 나타나고 자금·투자·대출·보험의 단일 잔액은 이름 오른쪽에서 불필요한 세로 공간을 만들지 않음
...
- 최초 금액 또는 일반 거래 이력이 있는 자산은 authoritative preview가 `ARCHIVE`를 반환하고, 보관 뒤 활성 그룹·활성 `n/50`·신규 거래·연결 계좌 선택기에서는 제외되지만 순자산·과거 거래·월간 통계에는 같은 금액으로 남음
- 결제·자동이체 계좌 또는 미처리 카드 결제 일정의 자금 출처인 자산은 blocker 종류·연결 자산명이 표시되고 최종 정리 행동이 제공되지 않으며, 단순 잔액과 미결제 명세는 warning일 뿐 차단하지 않음
- preview 뒤 다른 세션이 이력·잔액·연결·version을 바꾸면 apply는 `412`로 아무 정리도 반영하지 않고, dialog와 작성 중 상태를 유지한 채 `정리 결과가 달라졌어요`에 focus하고 `최신 내용 다시 확인`만 제공함
- 보관 목록은 compact 행과 `보관` 상태를 제공하고 보관 상세는 읽기 전용이며 복원·수정·새 거래 행동이 없음
- 자산 정리 dialog는 모바일 390px, iPad 세로·가로, 데스크톱에서 하나의 DOM과 같은 결과를 유지하고 가로 overflow 없이 `Escape`·브라우저 뒤로 가기로 닫힌 뒤 trigger focus를 복구함
...
- 일반 이체 선택기에는 음수 잔액을 포함한 활성 `BANK` 계좌만 표시되고 현금·신용카드·체크카드·적금은 표시되지 않으며, 계좌가 두 개 미만이면 저장할 수 없음
- 비계좌 자산을 출발 또는 도착으로 보낸 직접 API 요청은 `400 TRANSFER_BANK_ACCOUNT_REQUIRED`이고 거래·posting·잔액을 하나도 바꾸지 않음
...
- 거래 유형 변경, 신용카드 구매의 일반 수정·삭제, 자동 정산 거래의 일반 수정·삭제는 거부
...
- 모바일 거래 기록은 본문에 `가계부로 돌아가기` 전체 행을 두지 않고 sticky 상단 바에 접근 가능한 뒤로가기·유일한 `h1`을 표시
- floating 하단 4탭 dock은 휴대폰의 좌·우·하단 edge와 safe area에서 8px 이상 떨어지고 16px 이상 외곽 radius를 유지하며 현재 field·오류·저장·PWA 갱신 prompt를 가리지 않음
- 거래 금액·날짜는 320px부터 같은 행의 같은 폭·48px 높이로 보이고, 금액은 18~20px·semibold·tabular·우측 정렬·`원` 단위를 유지하며 18rem 미만 가용 폭에서는 한 열로 돌아감
...
- 모바일에서 theme를 바꾼 뒤 `768px` 이상으로 resize하거나 반대로 이동해도 새로 보이는 toggle의 icon·접근 가능한 이름·실제 theme가 공유 store와 일치
