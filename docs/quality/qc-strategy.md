...
- 결제·입금 자산과 보내는·받는 계좌, 카드 정정·자산 설정 연결 계좌가 공통 AssetPicker를 사용하고 모바일은 하단 drawer, `768px` 이상은 trigger에 붙는 popover로 열림
...
- 현재 사용자·다른 구성원·공동 소유 `BANK` 계좌가 이름과 소유 표시로 구분되고, 서로 다른 구성원 소유 계좌 사이 이체가 출발 `-amount`·도착 `+amount` posting으로 원자적으로 반영됨
...
- 거래 기록·수정 본문은 모든 viewport에서 중복 `가계부로 돌아가기` 링크 없이 제목부터 시작하고, 모바일만 sticky 상단 바에 접근 가능한 icon 뒤로가기·유일한 `h1`을 표시
...
- 가계부·자산 현황·월간 통계·가계부 설정의 `h1`은 모바일과 데스크톱 모두 24px semibold로 동일하며 breakpoint 전후에 화면별 크기가 달라지지 않음
