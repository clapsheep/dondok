...
활성 카테고리의 화면 순서는 방향별 `sort_order, id` 오름차순이다. 순서 변경은 같은 방향의 활성 카테고리 전체를 잠그고 요청한 ID 집합과 각 version이 모두 현재 상태와 같을 때만 10 단위 `sort_order`로 재배치한다. 일부 행만 적용하지 않으며 동시 추가·수정·삭제·재정렬이 있으면 전체를 `412`로 거부한다. 기존 `sort_order` 컬럼을 사용하므로 별도 순서 테이블이나 migration은 필요 없다.
