...
test('서버가 ARCHIVE를 결정하면 기록 유지와 신규 선택 제외·복원을 안내한다', () => {
  assert.match(removalDescription('ARCHIVE'), /기록과 잔액은 유지/)
  assert.match(removalDescription('ARCHIVE'), /새 거래에서는 숨겨/)
  assert.match(removalDescription('ARCHIVE'), /다시 사용할 수/)
})
