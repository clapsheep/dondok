  ...
  return (
    <section className="min-w-0 lg:pr-8" aria-labelledby="members-title">
      <div className="flex items-center gap-3">
        <UsersRound className="shrink-0 text-forest-700" size={24} aria-hidden="true" />
      ...
      <ul className="mt-5 divide-y divide-[var(--line)] border-y border-[var(--line)]">
        {ledger.members.map((member) => (
          <li key={member.memberId} className="flex min-h-14 items-center justify-between gap-3 px-1 py-3">
            <span className="flex min-w-0 items-center gap-3"><UserRound className="shrink-0 text-forest-700 dark:text-forest-100" size={20} aria-hidden="true" /><span className="truncate font-semibold">{member.displayName}</span></span>
